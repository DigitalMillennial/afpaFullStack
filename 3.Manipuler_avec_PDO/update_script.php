...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $artist_name = $_POST['artist_id']; // Мы будем искать ID по имени артиста
    $title = $_POST['title'];
    $year = $_POST['year'];
    $label = $_POST['label'];
    $genre = $_POST['genre'];
    ...
    // Получаем artist_id по имени артиста
    $artist_query = $record->prepare("SELECT artist_id FROM artist WHERE artist_name = ?");
    $artist_query->execute([$artist_name]);
    $artist_id = $artist_query->fetchColumn();
